...
function renderAircraftSelect(){
  $('registration').innerHTML='<option value="">Selecteer registratie…</option>'+config.aircraft.map((a,i)=>`<option value="${i}">${esc(a.registration)} · ${esc(a.type)}</option>`).join('');
...
function selectAircraft(){
  ...
  $('aircraftInfo').innerHTML=`<div><strong>Type</strong>${esc(a.type)}</div><div><strong>Fabrikant</strong>${esc(a.manufacturer)}</div><div><strong>Serienummer</strong>${esc(a.serialNumber)}</div>`;
  $('aircraftInfo').hidden=false;$('task').innerHTML='<option value="">Selecteer AMP-taak…</option>'+a.tasks.map((t,i)=>`<option value="${i}">${esc(t.label)}</option>`).join('');$('ampTask').value='';
}
...
function readEditor(){
  document.querySelectorAll('.aircraft-card').forEach(card=>{
    ...
    config.aircraft[i]={
      ...
      type:get('type'),manufacturer:get('manufacturer'),serialNumber:get('serialNumber'),
...
$('addAircraft').addEventListener('click',()=>{readEditor();config.aircraft.push({registration:'',type:'',manufacturer:'',serialNumber:'',tasks:[]});renderEditor()});
